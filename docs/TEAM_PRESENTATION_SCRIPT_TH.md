...
> เวอร์ชันปัจจุบันเปิดกล้องโทรศัพท์แบบสดได้ โดยขอกล้องหลังผ่าน browser, ไม่ขอเสียง, ไม่เปิดไฟช่วยถ่ายอัตโนมัติ และหยุดกล้องทันทีเมื่อปิดหน้ากล้องหรือซ่อนแท็บ แต่กล้องโทรศัพท์เปล่าเห็นเพียงตาภายนอก ไม่ใช่ fundus การถ่ายจอประสาทตาต้องใช้อะแดปเตอร์/condensing lens และผู้ปฏิบัติงานที่เข้าใจการจัดแนว ถ้าไม่ได้ยืนยันชุดถ่าย fundus เว็บจะบล็อกภาพจากกล้องไม่ให้เข้าโมเดล แม้ local model เชื่อมต่ออยู่
...
**ถาม: ตอนนี้เว็บใช้โมเดลได้จริงหรือยัง?**

ตอบ: ได้จริงเมื่อเปิดผ่าน local Python server บนเครื่องที่มี checkpoint หน้าเว็บจะเรียก `/predict` และคืน probabilities, provenance และ Grad-CAM จริง ส่วน GitHub Pages ที่เปิดจาก QR ไม่มี checkpoint/backend จึงใช้ได้เฉพาะกล้องและตรวจไฟล์ ยังไม่คืนผลโรค

**ถาม: ใช้โทรศัพท์ถ่ายตาโดยตรงแล้ววิเคราะห์ได้ไหม?**

ตอบ: เปิดกล้องและถ่ายได้ทางเทคนิค แต่ภาพกล้องมือถือเปล่าเป็นตาภายนอก ไม่ตรงกับ retinal fundus photographs ที่ใช้เทรน จึงห้ามส่งเข้าโมเดล ต้องมีอะแดปเตอร์ fundus หรือ condensing lens ที่เหมาะสม และยังต้องเพิ่ม quality gate/validation สำหรับอุปกรณ์นั้นก่อนอ้างประสิทธิภาพ

**ถาม: ทำไมโทรศัพท์เปิดเว็บแล้วเรียกโมเดลบนโน้ตบุ๊กไม่ได้?**

ตอบ: local server bind ที่ `127.0.0.1` เพื่อไม่เปิดโมเดลออกเครือข่าย `127.0.0.1` ของโทรศัพท์หมายถึงโทรศัพท์เอง ไม่ใช่โน้ตบุ๊ก การเชื่อมสองอุปกรณ์ต้องมี HTTPS gateway, device authorization และการทดสอบความปลอดภัยก่อน ไม่ควรเปลี่ยนเป็น `0.0.0.0` เพื่อความสะดวกอย่างเดียว
